...
CREATE TABLE tmp_community_posts_by_creationdate_postid AS
  ...
  FROM
    (
      SELECT
        creationdate,
        (CASE WHEN parentid IS NOT NULL THEN parentid ELSE id END) id,
...
CREATE TABLE community_posts_by_creationdate_postid AS
  SELECT
    ...
    questioncount,
    --viewcount,
    (SELECT MAX(tags) tags FROM tmp_community_posts_by_creationdate_postid src WHERE src.id=target.id) tags
  FROM
